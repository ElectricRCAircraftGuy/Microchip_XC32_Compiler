...
# Build to run on a Windows (mingw) host

"$SCRIPT_DIRECTORY/build-xc32-v4.35m.sh" "windows"
ret_code="$?"
if [ $ret_code -ne 0 ]; then
    echo "Error: Failed to build XC32m!"
    exit "$ret_code"
...
"$SCRIPT_DIRECTORY/build-xc32-v4.35c.sh" "windows"
ret_code="$?"
if [ $ret_code -ne 0 ]; then
    echo "Error: Failed to build XC32c!"
    exit "$ret_code"
