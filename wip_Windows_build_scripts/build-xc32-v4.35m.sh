...
windows_dir_suffix=""
extra_congigure_flags=""
if [ "$1" = "windows" ]; then
    windows_dir_suffix="_windows"
    # extra_configure_flags="--host=x86_64-w64-mingw32"
    extra_configure_flags="--host=x86_64-w64-ucrt"
elif [ "$1" != "" ]; then
...
BUILDDIR="$SCRIPT_DIRECTORY/xc32-v4.35-src/pic32m-build${windows_dir_suffix}"
# Where to install the compiler.
# Default: `INSTALLDIR=${HOME}/Downloads/build/opt`
INSTALLDIR="$SCRIPT_DIRECTORY/xc32-v4.35-src/installed/opt${windows_dir_suffix}/microchip/xc32_custom/v4.35"
